...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
    ...
    const statusText = useMemo(() => {
        ...
        return "Ciao! Sto usando LinkLine";
    }, [data])
                            ...
                            <div className='pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10'>
                                <Transition.Child as={Fragment} enter='transform transition ease-in-out duration-500' enterFrom='translate-x-full' enterTo='translate-x-0' leave="transform transition ease-in-out duration-500" leaveTo='translate-x-full'>
                                    <Dialog.Panel className="pointer-events-auto w-screen max-w-md">
                                        <div className='flex h-full flex-col bg-gradient-to-tr from-green-200 to-purple-200 py-6 shadow-xl'>
                                            <div className='px-4 sm:px-6'>
                                                <div className='flex items-start justify-end'>
                                                    <div className='ml-3 flex h-7 items-center'>
                                                        <button onClick={onClose} className='rounded-md bg-red-200 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2'>
                                                        ...
                                                        <dl className='space-y-8 px-4 sm:space-y-6 sm:px-6'>
                                                            ...
                                                            {data.isGroup && (
                                                                <div>
                                                                    ...
                                                                    <dd className='mt-4'>
                                                                        <ul className='flex flex-col max-h-72 overflow-y-auto gap-y-1'>
                                                                            {data.users.map(user => (
                                                                                <li key={user.id} className='flex items-center flex-row border-2 bg-slate-100 border-cyan-500 rounded-3xl gap-y-2 p-4'>
                                                                                    <Avatar user={user} />
                                                                                    <dl className='ml-4'>
